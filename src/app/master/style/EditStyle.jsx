    ...
    const [open, setOpen] = useState(false);
      ...
      const fetchStyleData = async () => {
        ...
        try {
          ...
          const response = await axios.get(
            `${BASE_URL}/api/fetch-style-by-Id/${styleId}`,
            {
          ...
          setFormData({
            style_type: styleData.style_type || "",
            style_status: styleData.style_status || "Active",
          });
          setOriginalData({
            style_type: styleData.style_type || "",
            style_status: styleData.style_status || "Active",
      ...
      const handleSubmit = async () => {
        if (!formData.style_type.trim()) {
          toast({
            title: "Error",
            description: "Style type is required",
            variant: "destructive",
          });
          return;
        }
        ...
        try {
          ...
          const response = await axios.post(
            `${BASE_URL}/api/update-style/${styleId}?_method=PUT`,
            formData,
            {
              headers: { Authorization: `Bearer ${token}` },
            }
      ...
      const hasChanges =
        ...
        (formData.style_type !== originalData.style_type ||
          formData.style_status !== originalData.style_status);
  return (
        ...
        {isFetching ? (
          ...
          <div className="grid gap-4">
            <div className="space-y-1">
              <h4 className="font-heading font-bold text-stone-800 leading-none">Edit Style</h4>
              <p className="text-xs text-stone-500">
                Update style name and active status
              </p>
            </div>
            <div className="grid gap-3">
              <div className="grid gap-1.5">
                <label htmlFor="style_type" className="text-xs font-semibold text-stone-700">
                  Style Name
                </label>
                      ...
                      <div className="absolute right-2.5 top-1/2 -translate-y-1/2">
                        <RefreshCcw
                          ...
                          onClick={() =>
                            setFormData((prev) => ({
                              ...prev,
                              style_type: originalData.style_type,
                            }))
                          }
                        />
                      </div>
              ...
              <div className="grid gap-1.5">
                <label
                  htmlFor="style_status"
                  className="text-xs font-semibold text-stone-700"
                >
                  Status
                </label>
                <Select
                  value={formData.style_status}
                  onValueChange={(value) =>
                    setFormData((prev) => ({
                      ...prev,
                      style_status: value,
