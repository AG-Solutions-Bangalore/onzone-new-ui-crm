...
const EditFairOrderForm = () => {
  ...
  useEffect(() => {
    if (responseData) {
      const orderHeader =
        ...
        responseData?.orderHeader ||
        responseData?.data ||
        {};
      const subs =
        responseData?.fairOrderFormSub ||
        responseData?.subs ||
        responseData?.orderSubs ||
        [];
      ...
      if (Array.isArray(subs) && subs.length > 0) {
        setSubItems(
          subs.map((s, index) => ({
            id: `loaded-${s.id || index}-${s.fair_order_sub_barcode}`,
            subId: s.id,
            fair_order_sub_barcode_main:
              s.fair_order_sub_barcode_main || s.fair_order_sub_barcode,
            fair_order_sub_barcode: s.fair_order_sub_barcode,
            fair_order_sub_barcode_type: s.fair_order_sub_barcode_type || "S",
            fair_order_sub_dress_type: s.fair_order_sub_dress_type || "S",
            fair_order_sub_dress_size: s.fair_order_sub_dress_size || "S-36",
            fair_order_sub_mrp: s.fair_order_sub_mrp || "0",
            fair_order_sub_quantity: parseInt(s.fair_order_sub_quantity, 10) || 1,
          }))
        );
      }
    }
  ...
  // Delete Sub Item from backend
  const deleteSubMutation = useMutation({
    mutationFn: async (subId) => {
      const token = localStorage.getItem("token");
      const res = await axios.delete(
        `${BASE_URL}/api/fairDeleteOrderFormSub/${subId}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
    ...
    onSuccess: (res, subId) => {
      ...
      toast({
        title: "Deleted",
        description: res?.msg || "Sub item deleted from order",
      });
    },
    onError: (err) => {
      toast({
  ...
  if (isOrderLoading) {
    return <LoaderComponent name="Order Details" />;
  }
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-[#FDFBF7] border border-stone-200/80 px-5 py-3 rounded-2xl shadow-2xs">
          <div>
            ...
            <p className="text-xs text-stone-500 font-medium">
              Update retailer details, edit size allocations, or add/remove garment barcodes.
            </p>
          </div>
                ...
                {retailerOptions.length > 0 && (
                  <div className="mb-1.5">
                    <Select
                      ...
                      placeholder="Search and change retailer..."
                      ...
                      value={
                        retailerOptions.find(
                          (opt) => opt.value === formData.fair_order_retailer
                        ) || null
                      }
                    />
                  </div>
